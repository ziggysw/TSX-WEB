...
exports = module.exports = function(server){
  ...
  server.get('/user/:id', function (req, res, next) {
    ...
    sql += "`pay_to_bank`, `have_card`, `have_account`, `kill`, `death`, `refere`, `timePlayedJob`, U.`skin`, UNIX_TIMESTAMP(`last_connected`) as `last_connected`, `vitality`, `level` as `rang`, `prestige`, F.`user_id` as `forum_id` "
    sql += " FROM `rp_users` U INNER JOIN `rp_jobs` J ON J.`job_id`=U.`job_id` INNER JOIN `rp_groups` G ON G.`id`=U.`group_id` LEFT JOIN `ts-x`.`phpbb3_users` F ON REPLACE(F.`steamid`, 'STEAM_0', 'STEAM_1')=U.`steamid` WHERE U.`steamid`=? ORDER BY F.`user_lastvisit` DESC LIMIT 1;";
