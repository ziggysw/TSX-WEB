...
exports = module.exports = function(server) {
  ...
  */
  server.post('/report/police', function (req, res, next) {
    try {
      if( !req.params['steamid'] || !req.params['timestamp'] || !req.params['reason'] || !req.params['moreinfo'] ) throw "InvalidParam";
      var d = new Date(req.params['timestamp']);
      if(!(d instanceof Date && isFinite(d))) throw "InvalidDate";
  ...
  */
  server.put('/report/:id', function (req, res, next) {
    try {
      if( req.params['id'] == 0 && req.params['lock'] === undefined ) throw "InvalidParam";
      server.conn.query(server.getAuthSteamID, [req.headers.auth], function(err, row) {
        if( err ) throw err;
        if( row[0] == null ) throw "NotAuthorized";
