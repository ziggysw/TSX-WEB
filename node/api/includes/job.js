...
server.put('/job/:id/approuve', function (req, res, next) {
  ...
  server.conn.query(server.getAuthSMAdmin, [req.headers.auth], function(err, row) {
    ...
    if( SteamID != "STEAM_1:0:7490757" && SteamID != "STEAM_1:1:79449241" && SteamID != "STEAM_1:1:39278818" ) return res.send(new ERR.NotAuthorizedError("NotAuthorized"));
...
server.get('/jobs/:id/capital/:scale', function (req, res, next) {
  ...
  sql += "	FROM `rp_sell` WHERE `steamid`='CAPITAL' AND `job_id`=? GROUP BY `date` ORDER BY `date` ASC;"
